...
function Button.new(window, position, size, text, callback)
...
function Label.new(window, position, text, size, centered)
...
function Slider.new(window, position, width, min, max, defaultValue, name, callback)
    ...
    -- Name label
    self.NameLabel = Label.new(window, self.Position, self.Name, 12)
    
    -- Value label
    self.ValueLabel = Label.new(window, 
...
function Checkbox.new(window, position, text, defaultValue, callback)
    ...
    -- Text label
    self.Label = Label.new(window, self.Position + Vector2.new(self.Size + 8, -2), self.Text, 12)
...
function Textbox.new(window, position, size, placeholder, callback)
...
-- Export the classes to FrostsUI
FrostsUI.Window = Window
FrostsUI.Button = Button
FrostsUI.Label = Label
FrostsUI.Slider = Slider
FrostsUI.Checkbox = Checkbox
FrostsUI.Textbox = Textbox

-- Utility Functions
function FrostsUI.CreateExampleUI()
    local window = Window.new("FrostsUI Example", Vector2.new(100, 100), Vector2.new(400, 500))
    
    -- Title label
    Label.new(window, Vector2.new(20, 50), "Welcome to FrostsUI!", 18)
    Label.new(window, Vector2.new(20, 80), "A complete UI library for LuaU", 12)
    
    -- Slider
    local slider = Slider.new(window, Vector2.new(20, 120), 300, 0, 100, 50, "Volume", function(value)
        print("Volume set to:", value)
    end)
    
    -- Checkbox
    Checkbox.new(window, Vector2.new(20, 170), "Enable Sound", true, function(checked)
        print("Sound enabled:", checked)
    end)
    
    Checkbox.new(window, Vector2.new(20, 200), "Show FPS", false, function(checked)
        print("Show FPS:", checked)
    end)
    
    -- Textbox
    Textbox.new(window, Vector2.new(20, 240), Vector2.new(200, 30), "Enter your name...", function(text)
        print("Name entered:", text)
    end)
    
    -- Buttons
    Button.new(window, Vector2.new(20, 290), Vector2.new(120, 35), "Apply", function()
        print("Settings applied!")
    end)
    
    Button.new(window, Vector2.new(160, 290), Vector2.new(120, 35), "Cancel", function()
        window:Destroy()
    end)
    
    -- Status label
    local statusLabel = Label.new(window, Vector2.new(20, 340), "Status: Ready", 12)
    
    -- Info button with custom color
    local infoBtn = Button.new(window, Vector2.new(20, 380), Vector2.new(100, 30), "Info", function()
        statusLabel:SetText("Status: Info button clicked!")
    end)
    infoBtn:SetColor(FrostsUI.Colors.Info)
    
    -- Warning button
    local warnBtn = Button.new(window, Vector2.new(140, 380), Vector2.new(100, 30), "Warning", function()
        statusLabel:SetText("Status: Warning button clicked!")
    end)
    warnBtn:SetColor(FrostsUI.Colors.Warning)
    
    -- Success button
    local successBtn = Button.new(window, Vector2.new(260, 380), Vector2.new(100, 30), "Success", function()
...
function FrostsUI.CreateWindow(title, width, height)
    ...
    return Window.new(title, Vector2.new(centerX, centerY), Vector2.new(width, height))
end
